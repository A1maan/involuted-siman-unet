...
train_loader_2018 = DataLoader(train_dataset_2018, batch_size=4, shuffle=True, drop_last=True, worker_init_fn=seed_worker)
test_loader_2018 = DataLoader(test_dataset_2018, batch_size=4, shuffle=False, worker_init_fn=seed_worker)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train on ISIC2018")
    ...
    best_loss = float("inf")
    best_model_path = os.path.join(weights_dir, f"best_{args.model}_unet_isic2018.pth")
    ...
    final_model_path = os.path.join(weights_dir, f"{args.model}_unet_isic2018.pth")
    ...
    imgs, masks = imgs.to(device), masks.to(device)
    with torch.no_grad():
        outputs = model(imgs)  # Returns single output
    ...
    plt.savefig(os.path.join(plots_dir, f'{args.model}_unet_predictions_grid_isic2018.png'))
    plt.show()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title(f"{class_name} - Loss Curve (ISIC2018)")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, f'{args.model}_unet_loss_curve_isic2018.png'))
    plt.show()
